...
def create_realistic_invoice():
    ...
    # Load products
    with open("Jsons/realistic_product_catalog_3000.json", "r") as f:
        products = json.load(f)

    # Load coupons
    with open("Jsons/coupon_codes.json", "r") as f:
        coupons = json.load(f)

    # Load store locations
    with open("Jsons/store_locations.json", "r") as f:
        stores = json.load(f)
...
def json_message_generator():
    invoice_sample = create_realistic_invoice()
    # print(invoice_sample)
    # Organize the payload
    final_payload = {
        "timestamp": invoice_sample["timestamp"],
        "timestamp_epoch_ms": invoice_sample["timestamp_epoch_ms"],
        "invoice": {k: v for k, v in invoice_sample.items() if k not in ("timestamp", "timestamp_epoch_ms")}
    }
